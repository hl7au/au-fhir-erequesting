...
class FSHTank {
            ...
            switch (t) {
                ...
                case Fishable_1.Type.ValueSet:
                    result = this.getAllValueSets().find(vs => (vs.name === base ||
                        ...
                        (0, common_2.getNonInstanceValueFromRules)(vs, 'name', '', 'name') === base) &&
                    ...
                    if (!result) {
                        result = this.getAllInstances().find(vsInstance => (vsInstance === null || vsInstance === void 0 ? void 0 : vsInstance.instanceOf) === 'ValueSet' &&
                            ((vsInstance === null || vsInstance === void 0 ? void 0 : vsInstance.usage) === 'Definition' || (vsInstance === null || vsInstance === void 0 ? void 0 : vsInstance.usage) === 'Inline') &&
                            ((vsInstance === null || vsInstance === void 0 ? void 0 : vsInstance.name) === base ||
                                ...
                                (0, common_2.getNonInstanceValueFromRules)(vsInstance, 'name', '', 'name') === base) &&
                            (version == null ||
                ...
                case Fishable_1.Type.CodeSystem:
                    result = this.getAllCodeSystems().find(cs => (cs.name === base ||
                        ...
                        (0, common_2.getNonInstanceValueFromRules)(cs, 'name', '', 'name') === base) &&
                    ...
                    if (!result) {
                        result = this.getAllInstances().find(csInstance => (csInstance === null || csInstance === void 0 ? void 0 : csInstance.instanceOf) === 'CodeSystem' &&
                            ...
                            ((csInstance === null || csInstance === void 0 ? void 0 : csInstance.name) === base ||
                                ...
                                (0, common_2.getNonInstanceValueFromRules)(csInstance, 'name', '', 'name') === base) &&
                            (version == null ||
    ...
    fishForMetadata(item, ...types) {
        ...
        if (result) {
            (0, common_1.applyInsertRules)(result, this);
            const meta = {
                id: result.id,
                name: result.name
            };
            if (result instanceof fshtypes_1.Profile ||
                ...
                const imposeProfiles = this.findExtensionValues(result, common_1.IMPOSE_PROFILE_EXTENSION, 'structuredefinition-imposeProfile', 'SDImposeProfile');
                ...
                const assignedUrl = (0, common_2.getNonInstanceValueFromRules)(result, 'url', '', 'url');
    ...
    hasLogicalCharacteristic(logical, code) {
        if (logical.characteristics.includes(code)) {
            return true;
        }
        else {
            const charValues = this.findExtensionValues(logical, common_1.TYPE_CHARACTERISTICS_EXTENSION, 'structuredefinition-type-characteristics', 'SDTypeCharacteristics');
            if (charValues.some(v => v.code === code)) {
                return true;
            }
            if (code === 'can-be-target') {
                const targetValues = this.findExtensionValues(logical, common_1.LOGICAL_TARGET_EXTENSION, 'logical-target', 'LogicalTarget');
    ...
    findExtensionValues(item, extensionUrl, extensionId, extensionName) {
        ...
        const matchingExtensionIndexers = [];
        const indexerToValueMap = new Map();
        item.rules.forEach(rule => {
            var _a, _b, _c;
            if (rule.path === '' && rule instanceof rules_1.CaretValueRule && ((_a = rule.caretPath) === null || _a === void 0 ? void 0 : _a.length)) {
                // Match on simple extension url or value rules using a numeric index
                const numericMatch = rule.caretPath.match(/^extension(\[\d+\])?\.(url|value[A-Z][A-Za-z0-9]*)$/);
                if (numericMatch) {
                    ...
                    const namedMatch = rule.caretPath.match(/^extension(\[([^\]]+)\](\[\d+\])?)\.value[A-Z][A-Za-z0-9]*$/);
                    if (namedMatch) {
                        ...
                        // namedMatch[3]: [1]
                        const name = (_b = this.resolveAlias(namedMatch[2])) !== null && _b !== void 0 ? _b : namedMatch[2];
                        if ([extensionUrl, extensionId, extensionName].includes(name)) {
                            const indexer = `[${extensionUrl}]${(_c = namedMatch[3]) !== null && _c !== void 0 ? _c : '[0]'}`;
                            matchingExtensionIndexers.push(indexer);
                            indexerToValueMap.set(indexer, rule.value);
                        }
                    }
