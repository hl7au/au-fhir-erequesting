...
Description: "This profile sets the minimum expectations for a CommunicationRequest resource used to represent a request for direct communication from a diagnostic provider (Filler) to a clinical provider for the purpose of providing urgent results.  
This communication is not a substitute for the formal diagnostic report, but is intended to ensure timely notification of a result.  
Phone is the supported communication channel, which is the typical medium for this use case.  
